...
function registerActions() {
    ...
    el("wifi_save_btn").onclick = function() {
        ...
        if (el("wifi_sta_connect_timeout").value != lastInfo.wifi_ap_pass) {
            data.sta_connect_timeout = parseInt(el("wifi_sta_connect_timeout").value);
        }
...
function swSetConfig(c) {
    ...
    let autoOffDelay = el(c, "auto_off_delay").value;
    let spinner = el(c, "save_spinner");
    let enable_ap_switch = el(c, "enable_ap_switch").checked;
    let sta_to_ap_change_delay_s = parseInt(el(c, "sta_to_ap_change_delay_s").value);
    ...
    let cfg = {
        ...
        enable_ap_switch: enable_ap_switch
    };
    if (autoOff) {
        cfg.auto_off_delay = dateStringToSeconds(autoOffDelay);
    }
    ...
    if (enable_ap_switch) {
        cfg.sta_to_ap_change_delay_s = sta_to_ap_change_delay_s;
    }
...
function findOrAddContainer(cd) {
    ...
    switch (cd.type) {
        ...
        case Component_Type.kLock:
            ...
            el(c, "enable_ap_switch").onchange = function(ev) {
              el(c, "enable_ap_switch_container").style.display = this.checked ? "block" : "none";
              markInputChanged(ev);
            };
            break;
...
function updateComponent(cd) {
    ...
    switch (cd.type) {
        ...
        case Component_Type.kLightBulb: {
            ...
            checkIfNotModified(el(c, "enable_ap_switch"), cd.enable_ap_switch);
            setValueIfNotModified(el(c, "sta_to_ap_change_delay_s"), cd.sta_to_ap_change_delay_s);
            el(c, "enable_ap_switch_container").style.display = el(c, "enable_ap_switch").checked ? "block" : "none";
            if (cd.apower !== undefined) {
                updateInnerText(
                    el(c, "power_stats"), `${Math.round(cd.apower)}W, ${cd.aenergy}Wh`);
                el(c, "power_stats_container").style.display = "flex";
            }
            ...
            el(c, "auto_off_delay_container").style.display =
                el(c, "auto_off").checked ? "flex" : "none";
            ...
            if (cd.state_led_en !== undefined) {
                if (cd.state_led_en == -1) {
                    ...
                    el(c, "state_led_en_container").style.display = "flex";
        ...
        case Component_Type.kWindowCovering: {
          ...
          avg power: ${cd.move_power} W`;
                el(c, "pos_ctl").style.display = "flex";
            } else {
                posText = "n/a";
...
function setUpdateInProgress(val) {
    ...
    let fwUplBtnTextEl = el("fw_file_text");
    if (val) {
        el("components").innerHTML = "";
        updateCommonVisibility(false);
    }
    fwUplBtnTextEl.innerText = val === true ? "Updating..." : "Update from file...";
    el("fw_spinner").className = val === true ? 'spin' : '';
    el("fw_upd_text").style.pointerEvents = val === true ? "none" : "auto";
