    ...
    case State::kMoving: {
      ...
      int moving_time_ms = (now - begin_) / 1000;
        ...
        if (moving_to_limit_pos && cfg_->man_cal) {
            ...
            LOG(LL_INFO, ("Moving to limit position ON with manual calibration ON. Divider = %.2f", moveDivider));
        }
        ...
        LOG(LL_INFO, ("POS_DIFF %.2f", pos_diff));
        // ADD LOG HERE
      float new_cur_pos =
      ...
      if (moving_to_limit_pos && !reverse) {
        if (cfg_->man_cal) {
            if (std::abs(tgt_pos_ - cur_pos_) > 1 || (moving_time_ms < cfg_->max_ramp_up_time_ms)) {
                // Still moving or ramping up.
                LOG(LL_INFO, ("STILL MOVING. TGT %d, CURR: %d. (NOW: %d, BEGIN: %d)", (int)tgt_pos_, (int)cur_pos_, now, begin_));
                break;
            } else {
            ...
            if (p > cfg_->idle_power_thr || (cfg_->man_cal && moving_time_ms <= too_long_time) || (std::abs(tgt_pos_ - cur_pos_) > 1) ||
