...
void WindowCovering::ExecuteIdleNoneState() {
    ...
    } else if (GetDesiredMoveDirection() != Direction::kNone) {
...
void WindowCovering::ExecuteRampUpState() {
    ...
    if (cfg_->man_cal || p >= cfg_->move_power * 0.75) {
        ...
        if (elapsed_us_ms > cfg_->max_ramp_up_time_ms) {
            LOG(LL_ERROR, ("[RampUp] Failed to start moving..."));
            tgt_state_ = State::kError;
            SetInternalState(State::kStop);
        }
    }
...
bool WindowCovering::IsTargetPositionReached(float precision) {
    return std::abs(tgt_pos_ - cur_pos_) <= precision;
}
...
void WindowCovering::ExecuteMovingState() {
    ...
    {
        LOG(LL_ERROR, ("[Moving] Obstruction: p = %.2f t = %d", p, moving_time_ms));
    ...
    if (moving_to_limit_pos && !reverse) {
        ...
        if (IsTargetPositionReached(0.5)) {
